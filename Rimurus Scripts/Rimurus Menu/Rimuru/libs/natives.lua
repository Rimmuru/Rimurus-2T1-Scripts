...
function HUD.SHOW_HUD_COMPONENT_THIS_FRAME(id)
    return native.call(0x0B4DF1FA60C0E664, id)
end
...
function NETWORK.NETWORK_SESSION_SET_MATCHMAKING_GROUP_MAX(type, count)
    native.call(0x8B6A4DD0AF9CE215, type, count)
end
...
function NETWORK.NETWORK_BAIL()
    return native.call(0x95914459A87EBA28)
end
...
function PLAYER.SET_PLAYER_WANTED_LEVEL(player, wantedLevel)
    return native.call(0x39FF19C64EF7DA5B, player, wantedLevel, false)
end
...
function CAM._INVALIDATE_VEHICLE_IDLE_CAM()
...
function VEHICLE.GET_VEHICLE_CUSTOM_PRIMARY_COLOUR(vehicle)
    local colour = {red=0, green=0, blue=0}
    ...
    colour.red = bufferR:__tointeger()
    colour.green = bufferG:__tointeger()
    colour.blue = bufferB:__tointeger()
...
function VEHICLE.GET_VEHICLE_CUSTOM_COLOUR(vehicle)
    return VEHICLE.GET_VEHICLE_CUSTOM_PRIMARY_COLOUR(vehicle), VEHICLE.GET_VEHICLE_CUSTOM_SECONDARY_COLOUR(vehicle)
end
...
function PED.SET_PED_ACCURACY(ped, accuracy)
    return native.call(0x7AEFB85C1D49DEB6, ped, accuracy)
end
...
function ENTITY.GET_ENTITY_ROTATION(entity, rotationOrder)
    return native.call(0xAFBD61CC738D9EB9, entity, rotationOrder):__tov3()
end
...
function TASK.TASK_THROW_PROJECTILE(ped, x, y, z)
    return native.call(0x7285951DBF6B5A51, ped, x, y, z, 0, 0)
end
...
function WEAPON.SET_WEAPON_DAMAGE_MODIFIER(weaponHash, damageMultiplier)
    return native.call(0x4757F00BC6323CFE, weaponHash, damageMultiplier)
end
...
function MISC.SET_BIT(address, offset)
    local bufAddr = native.ByteBuffer8()

    native.call(0x933D6A9EEC1BACD0, bufAddr, offset)
end